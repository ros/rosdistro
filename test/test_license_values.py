...
def test_license_values():
    with Fold():
        print("""Checking if the 'package.xml' files for all packages have some sane
value in license field.
""")
        ...
        for dist_name, cache in caches.items():
            ...
            for pkg_name, pkg_xml in cache.release_package_xmls.items():
                # Collect parsing warnings and fail if license value isn't valid
                warnings = []
                pkgs[pkg_name] = parse_package_string(pkg_xml, warnings=warnings)
                for warning in warnings:
                    if 'license value' in warning:
